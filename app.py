...
# ================= PLACEHOLDER GLOBAL (CLAVE REAL) =================
titulo_placeholder = st.empty()

# Render SIEMPRE el título si existe
if st.session_state.get("titulo_largo"):
    titulo_placeholder.header(st.session_state.titulo_largo)
    st.divider()
...
# ================= 3. TEST =================
    ...
    if st.session_state.idx < len(preguntas):
        ...
        st.write(p['q'])
        res = st.radio("Opciones:", p['ops'], key=f"radio_{st.session_state.idx}")
